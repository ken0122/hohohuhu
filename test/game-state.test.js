...
import { createGame, updateGame, speedForLevel } from "../src/renderer/game-state.js";
...
test("only clearing the entire round increases speed, preserving current direction", () => {
  ...
  assert.equal(game.level,2); assert.equal(game.pet.speed,300); assert.equal(game.pet.vx,300);
...
test("each new round is faster, stationary clears do not launch the pet, new games reset", () => {
  for (let level=1;level<100;level++) assert.equal(speedForLevel(level+1)-speedForLevel(level),20);
  const game=createGame(1200,800);

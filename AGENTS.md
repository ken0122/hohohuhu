...
- 聊天同入口、单条一次性提醒。使用已配置的对话模型返回紧凑 JSON 意图，不添加 provider 专属参数；160 token 预算和 15 秒超时不变。仅发送当前消息、必要纸条/澄清草稿及本地时间信息，不发送聊天历史。
- 模型提出意图，主进程校验并执行；本地保存成功后才确认。替换必须明确确认，旧请求和旧按钮不得复活已取消的纸条。
- 纸条以 safeStorage 加密保存于 userData，禁止明文降级。计时独立于 renderer；退出/休眠期间无法准时提醒，恢复后补提醒。
- 自动到期气泡不抢焦点、不覆盖聊天输入；手动隐藏和吃豆豆中仅发一次系统通知兜底。收下删除，关闭只收起；遵守减少动态效果与多语言主题。

...
app.whenReady().then(async () => {
  ...
  try {
    ...
    await until(() => js("Array.from(document.querySelectorAll('.mascot-svg')).every(s=>s.getAnimations().length>0)"));
    await until(() => js("new DOMMatrix(getComputedStyle(document.querySelector('#desktop svg')).transform).m42 < -.05"));
    ...
    assert.equal(await js("getComputedStyle(document.querySelector('#desktop svg')).animationName"), "happy-hop");

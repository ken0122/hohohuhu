...
状态栏菜单提供「外观」和「语言」两个单选子菜单。外观可跟随 macOS，也可固定为浅色或深色；聊天气泡、互动提示、聊天设置和角色库会实时同步，Pac-Man 保留固定深色夜幕。语言首次跟随系统首选语言，可手动选择简体中文、繁体中文、英语、日语、法语、德语或俄语；不支持的系统语言回退英语。切换无需重启，也不会重载窗口、清空聊天输入、角色编辑草稿或游戏进度。

中文界面显示「呼噜呼噜」，其他语言的界面显示 **Hulu Hulu**；安装包仍为「呼噜呼噜.app」，npm 包名、`bluepet` 命令和应用标识不变。菜单、窗口、辅助功能标签、通知、内置角色资料和新发起的角色图片分析会使用当前界面语言。用户自己添加或编辑的角色名称、人格与台词始终保留原文，不会在切换语言时被改写。

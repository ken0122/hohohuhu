...
| Pet · 互动与移动 | 留在身边，不自主散步；光标靠近会让路、快速逼近会闪开，进入角色区域后停下供互动；安静 12–22 秒后偶尔张望或伸展 | 鼠标互动；选 Pet 或点击后用方向键移动，松键停止；Esc 释放键盘焦点 |
...
所有内置、已添加和以后导入的角色共享两项标准动作：支持眼神的角色会在 Dodge、Pet、Pac-Man 和角色预览中看向光标；发生明确的水平移动时，形象会朝移动方向左右翻转，停下或仅上下移动时保留最后朝向。导入角色会在本地从已确认的眼睛框中定位眼白、原有固定瞳孔和眼白颜色，用同色遮片替换为可移动瞳孔，并按真实眼白余量分别计算横向与纵向视线范围，不改写原图；无法可靠定位时才使用受限的眼框或头部回退位置。

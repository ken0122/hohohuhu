...
所有内置、已添加和以后导入的角色共享两项标准动作：支持眼神的角色会在 Dodge、Pac-Man 和角色预览中看向光标；Pet 中光标静止超过 5 秒后瞳孔回中放松，光标再次移动 1px 即恢复注视。发生明确的水平移动时，形象会朝移动方向左右翻转，停下或仅上下移动时保留最后朝向。导入角色会在本地从已确认的眼睛框中定位眼白、原有固定瞳孔和眼白颜色，用同色遮片替换为可移动瞳孔，并按真实眼白余量分别计算横向与纵向视线范围，不改写原图；无法可靠定位时才使用受限的眼框或头部回退位置。
...
状态栏菜单依次提供「角色…」「外观」「语言」「快捷键」，位于同一分区；所有带按键的操作都收在「快捷键」二级菜单。外观可跟随 macOS，也可固定为浅色或深色；聊天气泡、互动提示、聊天设置和角色库会实时同步，Pac-Man 保留固定深色夜幕。语言首次跟随系统首选语言，可手动选择简体中文、繁体中文、英语、日语、法语、德语或俄语；不支持的系统语言回退英语。切换无需重启，也不会重载窗口、清空聊天输入、角色编辑草稿或游戏进度。

中文界面显示「呼噜呼噜」，其他语言的界面显示 **Hulu Hulu**；安装包仍为「呼噜呼噜.app」，npm 包名、`bluepet` 命令和应用标识不变。菜单、窗口、辅助功能标签、通知、内置角色资料、新发起的角色图片分析，以及宠物自主和互动气泡都会使用当前界面语言。用户自己添加或编辑的角色名称、人格与台词仍保留原文；非当前语言下的气泡使用应用内置安全短句，不会回写或改写角色资料。
...
隐藏时先蓄力鼓起、裂开，再散成高对比蓝白碎片与两层扩散光环，动画约 420ms；主进程以 460ms 兜底隐藏窗口。开始隐藏即释放鼠标交互，期间再次恢复或选模式会取消消散；系统开启「减少动态效果」时直接隐藏。Pet、Dodge、聊天和 Pac-Man 均支持。
...
切换角色后，聊天系统提示词会由主进程根据当前角色已校验的人格资料生成；renderer 不能自行指定人格。导入新角色时另用 **`deepseek-v4-flash-vision-exp`** 分析图片质量、气质、互动语言、彩蛋建议和部件框，和日常聊天模型相互独立。

从状态栏菜单打开「聊天设置…」，填写 API 密钥和接口地址后保存，下次聊天立即生效，无需重启。接口地址支持 `https://api.deepseek.com` 或 `https://api.deepseek.com/anthropic`，统一使用 Anthropic 兼容接口，不支持第三方代理。保存不会发起连接测试。
...
- **聊天连不上？** 可先从菜单「聊天设置…」填写有效的 DeepSeek 官方配置；当前版本不支持任意 Claude provider。不要把密钥粘贴到 issue 或日志中。
...
从状态栏菜单打开「角色…」，选择呼噜呼噜、黑猫、小太阳或自己添加的角色。点击列表只预览，点击「使用角色」才切换。添加图片会创建一个新角色，不会覆盖正在使用的黑猫或其他角色；自己添加的角色可编辑名称、气质、互动台词、彩蛋和部件，也可删除，内置角色不能编辑或删除。桌面、Dodge 和 Pac-Man 共用选择；切换不会清空聊天输入、重开游戏或唤醒手动隐藏的宠物。角色窗口是普通 macOS 窗口，不会长期置顶遮挡其他应用。

导入流程：

1. 点击「添加角色…」，在系统文件选择器中选一张图片。选择器会明确提示图片将发送给 DeepSeek 分析。
2. 本地 Worker 规范化图片；DeepSeek 同时给出质量、气质、互动语言、专属彩蛋和部件框草稿。`reject` 会阻止保存，`warn` 允许用户核对后继续。
...
模型结论只是可编辑建议，不等同于像素级分割或骨骼识别。部件框会调整摸头、挠痒等命中区域、部分动作原点和无腿角色的下轮廓位移范围；彩色平面形象仍以整图呈现，不会凭空生成可独立摆动的尾巴、四肢或眼皮。所有人格、短句、动作名、时长、彩蛋触发和部件坐标都经过白名单与长度/范围校验，不接受脚本、CSS 或任意选择器。

最多保存 20 个本地角色。新增、编辑、选择和删除都会原子写入 userData 的 `characters-v2.json`；其中只保存名称、受限 SVG、已确认的分析资料和当前选择。旧 `characters-v1.json` 可只读兼容，并在下次修改时写入 v2，旧文件不自动覆盖。原图和原路径不保存，聊天与密钥也不进入角色数据。图片只在用户明确选择时发送一次给 DeepSeek 分析，不会公开分享。删除正在使用的角色会恢复呼噜呼噜，原图不受影响。

### SVG 角色架构

角色按「受限素材 → 已校验分析 → 人格/互动配方 → 部件绑定 → 动作控制」分层：`character-analysis.js` 约束多模态结果，`character-profile.js` 生成纯数据互动协议，`characters.js` 组合可信定义；renderer 的 `character.js` 负责绑定和替换，`pet-interactions.js` 负责按角色隔离的语言轮换、冷却和声明式彩蛋，`character-motion.js` 统一处理动作、暂停、减少动态效果与销毁。`mascot.js` 为 Pet/Pac-Man 提供稳定控制接口，切换时替换内部 SVG、销毁旧动作并恢复当前动作状态；隐藏粒子另行刷新取样缓存。原角色源 SVG、尺寸与三种模式不变；原生窗口移动不由角色动画接管。
...
基础 SVG 绑定不要求素材自带可操作的眼睛或特定身体路径，可使用整体移动起伏和通用互动动作；应用会为导入角色生成受限的运行时瞳孔层。首个黑猫样本由用户提供的 PNG 本地转换，原图与随项目分发的授权记录保存在 `assets/characters/black-cat/`。转换只生成静态路径；应用在可信的黑猫运行时绑定中叠加两枚会随指针转动的瞳孔，不改写生成的 SVG，尾巴也没有被自动拆分。小太阳由彩色导入桌面测试样例整理成项目原创 SVG，保留黄色身体、橙色耳朵、奶黄色眼白和珊瑚色动态瞳孔；它使用明快短句，并以连续戳三次触发「三连点亮」。Pet 抚摸反馈会按角色使用对应符号与颜色，黑猫为暖金色，小太阳为橙金色星光。默认桌面角色仍是蓝色单眼宠物。
...
项目原创小太阳素材见 `assets/characters/sunny-yellow/NOTICE.md`，随本仓库按 MIT License 分发。

...
眼睛默认完全睁开，每隔 3.8–7.2 秒眨眼约 180ms；亲昵眼部反应约 280ms 后恢复睁开。Pet 接近或 hover 时上移、微转并轻微压缩；默认角色以三颗蓝色爱心错峰飘起，角色绑定可为同一抚摸反馈提供自己的符号、颜色与阴影，黑猫使用暖金色的星光与爱心，小太阳使用橙金色星光与太阳。无互动、无移动且光标不靠近时，间隔 12–22 秒偶尔张望或伸展。摸头、挠痒、戳肚子、贴贴和长按抱抱有独立短反馈。

支持眼神的角色在 Dodge、Pet、Pac-Man 和角色预览中始终朝向光标；桌面以实际窗口中的眼睛锚点计算，游戏以角色当前位置计算。光标视线按帧更新、不叠加 CSS 追赶过渡，并独立于身体速度、聊天停稳和菜单状态。所有角色在明确水平移动时镜像朝向移动方向，静止或纯垂直移动时保留最后朝向；镜像位于独立运行时层，瞳孔用反向局部偏移补偿，最终屏幕视线不会颠倒。Dodge 点击穿透且持续可见，快速逼近触发带冷却与衰减的弹射；Pet 不自主散步，复用无漫游的靠近让路与快速反射，指针进入角色区域后立即停下，把优先级交还给摸头、长按和拖拽；方向键始终覆盖自动避让。Pet / Dodge 切换保留惯性，回到上次 Pet 停留位置时减速。拖拽阈值为 6px、长按约 650ms，拖拽不得兼触点击或抱抱；聊天、手动隐藏与模式切换需结束当前拖拽。

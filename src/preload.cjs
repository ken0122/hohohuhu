...
contextBridge.exposeInMainWorld("bluepet", {
  ...
  viewReminder: (view) => ipcRenderer.invoke("reminder:view", view),
  reminderAction: (value) => ipcRenderer.invoke("reminder:action", value),
  onReminder: (callback) => ipcRenderer.on("reminder:state", (_event, value) => callback(value)),
  onReminderError: (callback) => ipcRenderer.on("reminder:error", (_event, key) => callback(key)),
